...
| Docker-integration | `vitest run` + `test/fixtures/docker-compose.sftp.yml` (`atmoz/sftp`, port 2223), enabled by `GANGWAY_SFTP` (`test/*.integration.test.ts`: remoteOps hard delete/direct overwrite, mapping sync, sync walk) | Real `ssh2-sftp-client` semantics the mocks cannot express (rename-exists, partial `fastGet`, `stat` sizes, numeric SFTP error codes) | Required locally for transfer changes; enforced in CI's `e2e` job (`GANGWAY_SFTP=1 pnpm test` against the same fixture the e2e run then uses), must be green. Run them by hand with the fixture up while iterating |
